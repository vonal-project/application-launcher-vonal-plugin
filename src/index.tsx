...
let load_cache = () => {
    applicationCacher.cache().then((given_indices) => {
        ...
        indices.forEach((idx: AppIndex) => {
            idx.fuzzybuzz =
                idx.name + idx.exec + idx.genericName + idx.keywords.join(",");
        });
    });
};
...
export default async (q) => {
    if (q.match(/^[a-zA-Z]+(\s.*)?$/)) {
        let results = (await getResults(q))
            ...
            .map((fuzzyInfo) => (
                <ApplicationLauncherRow fuzzyInfo={fuzzyInfo} query={q} />
            ));

        return [
            ...results,
            <div className="row">
                <button onClick={load_cache} style={{ opacity: 0.5 }}>
                    Reload application cache
                </button>
                <div style={{ float: "right", paddingRight: "0.5rem" }}>
